...
export default function AdminDashboardPage() {
  ...
  const projectCount = mockProjectData.length;
  const employeeCount = mockEmployeeData.filter(e => e.active).length;
  const candidateCount = mockCandidates.length;
    ...
    <div className="admin-dashboard-gradient min-h-screen p-4 sm:p-8">
      ...
      <main>
        <div className="mb-8 grid gap-4 md:grid-cols-3">
            <KpiCard title="Total Projects" value={projectCount} icon={FolderKanban} link="/admin/projects" />
            <KpiCard title="Active Employees" value={employeeCount} icon={Users} link="/admin/employees" />
            <KpiCard title="Total Candidates" value={candidateCount} icon={UserPlus} link="/admin/candidates" />
        </div>
...
const KpiCard = ({ title, value, icon: Icon, link }: { title: string; value: string | number; icon: React.ElementType; link: string }) => {
    return (
        <Card>
