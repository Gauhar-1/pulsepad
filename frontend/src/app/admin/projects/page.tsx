...
export default function AdminProjectsPage() {
  ...
  const filteredProjects = useMemo(() => {
    ...
    if (activeTab === 'stock') {
      return searchFiltered.filter((p) =>
        (Array.isArray(p.tags) ? p.tags : []).map((t) => (t ?? '').toLowerCase()).includes('stock'),
      );
    }
  ...
  const activeLeads = useMemo(() => employees.filter(e => e.status === 'ACTIVE' && e.type === 'Lead'), [employees]);
  const activeVAs = useMemo(() => employees.filter(e => e.status === 'ACTIVE' && e.type === 'VA'), [employees]);
  const activeFreelancers = useMemo(() => employees.filter(e => e.status === 'ACTIVE' && e.type === 'Freelancer'), [employees]);
  const activeCoders = useMemo(() => employees.filter(e => e.status === 'ACTIVE' && e.type === 'Coder'), [employees]);
  const activeCoreEmployees = useMemo(() => employees.filter(e => e.status === 'ACTIVE' && e.type === 'Core'), [employees]);
