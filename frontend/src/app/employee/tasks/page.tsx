...
import { DailyAssessment, AssessmentTemplate, ChecklistItem } from '@/lib/definitions';
...
const fetchTasks = async (): Promise<{
  ...
  const res = await apiFetch('/employee/assessments');
...
export default function EmployeeTasksPage() {
  ...
  const template = useMemo(() => {
    ...
    return templates.find((t) => (t._id ?? (t as AssessmentTemplate & { id?: string }).id) === todaysAssessment.templateId);
  }, [templates, todaysAssessment]);

  const getChecklistId = (item: ChecklistItem & { id?: string }) => item._id ?? item.id ?? '';

  const initialTasks = useMemo(() => {
    if (!template || !todaysAssessment) return [];
    return template.checklist.map((item) => {
      const checklistId = getChecklistId(item as ChecklistItem & { id?: string });
      return {
        id: checklistId,
      text: item.text,
      isCompleted:
          todaysAssessment.responses.find((r) => r.checklistItemId === checklistId)
            ?.answer || false,
      };
    });
      ...
      queryClient.setQueryData(['employeeTasks'], (oldData: any) => {
        ...
        return {
          ...
          assessments: oldData.assessments.map((a: DailyAssessment) =>
            a._id === updatedData._id ? updatedData : a
          ),
        };
      });
    },
