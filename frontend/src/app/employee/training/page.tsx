...
const fetchTrainingTasks = async (): Promise<TrainingTask[]> => {
    const res = await apiFetch('/employee/training');
    if (!res.ok) {
...
export default function TrainingPage() {
  const { data: trainingTasks, isLoading } = useQuery<TrainingTask[]>({
  ...
  return (
    <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-6">
      ...
      <div className="grid gap-6">
        {isLoading ? (
            Array.from({length: 4}).map((_, i) => <TrainingCardSkeleton key={i} />)
        ) : (
